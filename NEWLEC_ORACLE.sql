...
--044   CHECK CONSTRAINT USING REGULAR EXPRESSION
-- 체크 제약 조건을 더 세밀하게 사용하기 위하여 정규식을 같이 사용
SELECT * FROM USER_CONSTRAINTS WHERE TABLE_NAME='MEMBER';
ALTER TABLE MEMBER DROP CONSTRAINT MEMBER_PHONE_CHK1;    -- 기존의 제약 조건 삭제
ALTER TABLE MEMBER ADD CONSTRAINT MEMBER_PHONE_CHK1 CHECK(REGEXP_LIKE(PHONE, '^01[016789]-\d{3,4}-\d{4}$'));  -- 정규식을 이용한 연락처 체크 제약 조건 추가, 시작과 끝을 나타내는 ^$ 문자 추가

 
--045   ENTITY CONSTRAINT
-- PRIMARY KEY / UNIQUE
-- 중복된 레코드가 없도록 제한
-- PRIMARY KEY : 기본키, 식별키, 식별컬럼이라 함, NULL을 허용하지 않음
-- UNIQUE : NULL을 허용

-- 테이블 생성할 때 PRIMARY KEY, UNIQUE 제약 조건 적용
CREATE TABLE TEST(
    ID VARCHAR2(50) CONSTRAINT NOTICE_ID_PK PRIMARY KEY,
    EMAIL VARCHAR2(200) CONSTRAINT NOTICE_ID_UQ UNIQUE,
    PHONE CHAR(13) CHECK(PHONE LIKE '010-%-____') NOT NULL
);  -- 컬럼 입력 시 함께 적용
...
CREATE TABLE TEST(
    ...
    CONSTRAINT TEST_EMAIL_UQ UNIQUE(EMAIL)
)   -- 컬럼명 입력과 별도로 하단에 적용

-- 테이블 생성 후 PRIMARY KEY, UNIQUE 제약 조건 적용
